...
FROM bde2020/hadoop-datanode:2.0.0-hadoop2.7.4-java8 AS base
...
# Fix old package sources. Now they are in archive.debian.org host
...
RUN apt-get update && \ 
    apt-get -y install git g++-4.9 make vim --no-install-recommends --force-yes 
